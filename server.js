...
const { getUploadsRootDir, ensureUploadDirectories } = require('./utils/storagePaths');
...
// Shared messagebox used by Login and Dashboard.
app.get('/messagebox.js', (req, res) => {
  res.sendFile(__dirname + '/messagebox.js');
});
...
// Login assets MUST be served publicly BEFORE Dashboard static middleware.
// Dashboard and Login both had a 'responsive.css' — Login now uses 'auth-responsive.css'
// to avoid the naming collision. Login assets are served here without session checks.
app.use(express.static('Login'));

// Protected dashboard files — require valid session.
// Without a valid server session, direct URL/back-button access goes to login.
