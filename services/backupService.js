...
const { getResidentUploadDir } = require('../utils/storagePaths');
...
class BackupService {
  ...
  getDefaultBackupPath() {
    ...
    if (process.env.BACKUP_PATH && process.env.BACKUP_PATH.trim()) {
      return path.resolve(process.env.BACKUP_PATH.trim());
    }
    if (process.env.RENDER_DISK_MOUNT_PATH && process.env.RENDER_DISK_MOUNT_PATH.trim()) {
      return path.join(path.resolve(process.env.RENDER_DISK_MOUNT_PATH.trim()), 'backups');
    }
   ...
   *
   * Important for Windows: path.resolve() on Linux/macOS does not recognize
   * strings like C:\\Users\\... as absolute paths. During development or when
   * a config file is copied between machines, a Windows path can accidentally
   * become /some/project/C:\\Users\\... . This method sanitizes that case so
   * the app does not try to create invalid folders such as:
   * C:\\mnt\\data\\work\\barangay-system\\C:\\Users\\...
   */
  resolvePath(backupPath) {
    ...
    let expandedPath = String(backupPath).trim()
      .replace(/^~(?=$|[\\/])/, os.homedir())
    ...
    // Repair malformed paths that contain an embedded Windows absolute path.
    // Example: /mnt/data/work/barangay-system/C:\Users\depaz\Documents\...
    const embeddedWindowsPath = expandedPath.match(/[a-zA-Z]:[\\/].*$/);
    if (embeddedWindowsPath) {
      expandedPath = embeddedWindowsPath[0];
    }

    // Preserve native Windows absolute paths instead of prefixing the project path.
    if (path.win32.isAbsolute(expandedPath)) {
  ...
  /**
   * Resident profile photos are stored on disk under uploads/residents while
   * the members collection stores the saved URL/path (for example
   * /uploads/residents/memberId-123.jpg). These helpers include the actual
   * image files in local backup archives and restore them with the database.
   */
  getResidentPhotosSourceDir() {
    return getResidentUploadDir();
  }

  getSafeResidentPhotoFileName(photoPath) {
    if (!photoPath) return '';
  ...
  getReferencedResidentPhotoNames(members = []) {
    const names = new Set();
    for (const member of members) {
      const fileName = this.getSafeResidentPhotoFileName(member && member.photo);
      if (fileName) names.add(fileName);
    }
    return names;
  }

  exportResidentPhotosToBackup(backupDir, members = []) {
    const sourceDir = this.getResidentPhotosSourceDir();
    const residentPhotosDir = path.join(backupDir, 'resident-photos');
    const filesDir = path.join(residentPhotosDir, 'files');
    fs.mkdirSync(filesDir, { recursive: true });

    const referencedNames = this.getReferencedResidentPhotoNames(members);
    const errors = [];
    const exportedFiles = [];
    let exportedBytes = 0;

    if (!fs.existsSync(sourceDir)) {
      fs.writeFileSync(path.join(residentPhotosDir, 'manifest.json'), JSON.stringify({
        included: true,
        source: 'uploads/residents',
        createdAt: new Date().toISOString(),
        referencedByDatabase: referencedNames.size,
        exportedFiles: 0,
        exportedBytes: 0,
        files: [],
        errors: [{ error: 'Resident photo folder does not exist yet.' }]
      }, null, 2));

      return {
        included: true,
        folder: 'resident-photos/files',
        referencedByDatabase: referencedNames.size,
        exportedFiles: 0,
        exportedBytes: 0,
        errors: [{ error: 'Resident photo folder does not exist yet.' }]
    ...
    const diskFiles = fs.readdirSync(sourceDir).filter(file => {
      if (!/\.(jpe?g|png|webp)$/i.test(file)) return false;
      const fullPath = path.join(sourceDir, file);
      return fs.statSync(fullPath).isFile();
    });

    // Include both DB-referenced photo files and any extra resident photo files
    // present on disk, so no existing resident photos are accidentally omitted.
    const namesToExport = new Set([...referencedNames, ...diskFiles]);

    for (const rawName of namesToExport) {
      const fileName = this.getSafeResidentPhotoFileName(rawName);
      if (!fileName) continue;

      const sourceFile = path.join(sourceDir, fileName);
      const destFile = path.join(filesDir, fileName);
      if (!fs.existsSync(sourceFile)) {
        errors.push({ fileName, error: 'Photo is referenced in the database but missing from uploads/residents.' });
        continue;
      }
      ...
      try {
        ...
        exportedFiles.push({
          fileName,
          size,
          referencedByDatabase: referencedNames.has(fileName)
        });
      } catch (error) {
  ...
  restoreResidentPhotosFromBackup(backupContentDir) {
    const residentPhotosDir = path.join(backupContentDir, 'resident-photos');
    const filesDir = path.join(residentPhotosDir, 'files');
    const targetDir = this.getResidentPhotosSourceDir();
    const errors = [];
    let restoredFiles = 0;
    let restoredBytes = 0;

    if (!fs.existsSync(filesDir)) {
      return { restored: false, files: 0, bytes: 0, reason: 'No resident photo files found in this backup.' };
    }

    fs.mkdirSync(targetDir, { recursive: true });

    // The database is restored from this same backup, so replace the photo
    // folder contents to keep file paths and database records synchronized.
    for (const file of fs.readdirSync(targetDir)) {
      const fullPath = path.join(targetDir, file);
      try {
        if (fs.statSync(fullPath).isFile()) fs.unlinkSync(fullPath);
      } catch (error) {
        errors.push({ fileName: file, error: `Could not remove old photo: ${error.message}` });
      }
    }

    for (const file of fs.readdirSync(filesDir)) {
      const safeName = this.getSafeResidentPhotoFileName(file);
      if (!safeName) continue;

      const sourceFile = path.join(filesDir, safeName);
      const targetFile = path.join(targetDir, safeName);
      try {
        if (!fs.statSync(sourceFile).isFile()) continue;
        fs.copyFileSync(sourceFile, targetFile);
        restoredFiles += 1;
        restoredBytes += fs.statSync(targetFile).size;
      } catch (error) {
        errors.push({ fileName: safeName, error: error.message });
      }
    }
  ...
  async createDatabaseOnlyBackupArchive(timestamp, backupData, metadata) {
    ...
    fs.writeFileSync(path.join(dbOnlyDir, 'metadata.json'), JSON.stringify({
      ...
      residentPhotos: {
        included: false,
        reason: 'Resident photo files are local-backup-only; member.photo paths remain in the database copy.'
      },
      fileManager: {
        included: false,
  ...
  async createBackup() {
    ...
    try {
      ...
      console.log('Exporting resident photos to local backup...');
      const residentPhotosExport = this.exportResidentPhotosToBackup(backupDir, localBackupData.members || []);
      ...
      const metadata = {
        ...
        residentPhotos: {
          ...residentPhotosExport,
          localOnly: true,
          note: 'Resident profile photo files from uploads/residents are included in the local PC/server backup archive.'
        },
        fileManager: {
          included: true,
          localOnly: true,
      ...
      try {
        ...
        databaseBackup = await this.saveBackupArchiveToDatabase(db, dbOnlyArchive, fileName, {
          ...
          residentPhotos: {
            included: false,
            localOnly: true,
            note: 'Excluded from MongoDB backup copy. Resident photo files are stored only in the local PC/server backup archive; the database copy keeps the member.photo path.'
          },
          fileManager: {
            included: false,
            localOnly: true,
      ...
      return {
        ...
        residentPhotos: metadata.residentPhotos,
  ...
  async restoreBackupFromArchive(backupFilePath, displayName) {
    ...
    try {
      ...
      const residentPhotosRestore = this.restoreResidentPhotosFromBackup(backupContentDir);
