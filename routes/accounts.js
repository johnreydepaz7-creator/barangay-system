...
async function findAccountByPhone(phone) {
  ...
  let account = await Account.findOne({ phone: { $regex: `^${escaped}$`, $options: 'i' } });
  if (!account) {
    account = await Account.findOne({ phone: { $regex: `^0${escaped}$`, $options: 'i' } });
  }
  return account;
}
...
router.post('/register', async (req, res) => {
  try {
    ...
    // Validation
    if (!lastName || !firstName || !phone || !role) {
    ...
    const normalizedPhone = phone.trim().replace(/\s+/g, '').replace(/^0/, '');

    // Check if phone already exists (case-insensitive, with flexible spacing)
    ...
    // If registering as Punong Barangay, remove any previous Punong Barangay
    if (role === 'Punong Barangay') {
      await Account.deleteMany({ role: 'Punong Barangay' });
    }
...
router.post('/admin/add', async (req, res) => {
  try {
    ...
    // If adding as Punong Barangay, remove any previous Punong Barangay
    if (role === 'Punong Barangay') {
      const deleteResult = await Account.deleteMany({ role: 'Punong Barangay' });
      replacedOldPunong = deleteResult.deletedCount > 0;
    }
...
router.put('/profile/phone', async (req, res) => {
  try {
    ...
    const normalizedCurrent = currentPhone.trim().replace(/\s+/g, '').replace(/^0/, '');
    const normalizedNew = newPhone.trim().replace(/\s+/g, '').replace(/^0/, '');
    ...
    // Find account by current phone and update
    const account = await Account.findOne({ 
      phone: { $regex: `^${normalizedCurrent.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, $options: 'i' }
    });
